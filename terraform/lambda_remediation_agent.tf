...
resource "aws_lambda_function" "remediation_agent" {
  ...
  # Each finding is an Anthropic call plus a synchronous iac-scanner
  ...
  timeout = 900
  # I/O-bound, so no reason to buy the extra CPU iac-scanner needs.
  memory_size = 512
  ...
  environment {
    variables = {
      ...
      IAC_SCANNER_FUNCTION_NAME = aws_lambda_function.iac_scanner.function_name
