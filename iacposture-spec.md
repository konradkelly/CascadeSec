...
```
                    ...
                    │      iac-scanner Lambda: Trivy + Checkov      │
                    │   (v1)                                          │
...
|---|---|---|
| **API Gateway (REST or HTTP API)** | Public-facing endpoints for the review dashboard (fetch findings, submit review decisions) and the GitHub webhook receiver. *HTTP API. `POST /github/webhook` (v3) is the one route with no JWT authorizer: GitHub cannot present a Cognito token, so its authentication is the HMAC checked in `webhook-receiver`, and it is throttled.* | Deployment, Security |
| **Lambda — `webhook-receiver`** | Verifies the GitHub App's webhook signature (HMAC over the raw body) and starts one pipeline execution per PR head. Holds only the webhook secret and `states:StartExecution`. *Built 2026-09-24 (v3); originally sketched as pulling the PR's files and queueing them on SQS -- see the SQS row and [`docs/ci-integration-spec.md`](./docs/ci-integration-spec.md) §3.* | Development with AWS Services |
| **Lambda — `iac-scanner`** | Runs Trivy + Checkov against an IaC snapshot — Terraform and OpenTofu today, one scanner for all languages (§4.3, docs/multi-iac-spec.md §3). Packaged as a container image in ECR. Named `terraform-scanner` until 2026-09-16. **v1.** | Development, Deployment |
...
**Decided 2026-09-13: a container image for the scanner; zip + layer for everything else.** `lambda/iac-scanner/Dockerfile`, built and pushed by `build-image.sh` to an ECR repository Terraform owns, and pinned by digest in `lambda_iac_scanner.tf` so a push followed by an apply rolls the function and an apply alone changes nothing.
...
3. **`iac-scanner` Lambda** runs Trivy + Checkov, writes raw findings to **DynamoDB** (`status: raw`), each tagged with `target_type` and `finding_class` (§5)
...
### 8.2 `iac-scanner`: remaining v1 work
...
`lambda/iac-scanner/fixtures/vulnerable-sample` with only
