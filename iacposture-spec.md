...
| **Lambda — `terraform-scanner`** | Runs Trivy + Checkov against Terraform files. Packaged with a shared Checkov Lambda layer + a Trivy-binary layer (tfsec until 2026-09-12; §8.2 item 6). **v1.** | Development, Deployment |
...
- **`terraform-scanner`-only layer (v1):** Trivy (single static Go binary, checks bundle embedded; replaced tfsec, see §8.2 item 6). At ~161MB unzipped it leaves ~7MB under the 250MB function+layers ceiling next to the ~86MB Checkov layer — the number to watch on any bump
